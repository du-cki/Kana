...
class EmbeddedPaginator(discord.ui.View):
    ...
    def __init__(self, ctx: commands.Context, source: list, per_page: int = 10, *, title: str = None, timeout: int = 180) -> None:  # type: ignore
        ...
        self.source: typing.List[typing.Any] = list(
            discord.utils.as_chunks(source, per_page)
        )
    ...
    def build_embed(self, page: int) -> discord.Embed:
        return discord.Embed(
            title=self.title, description="\n".join(self.source[page]), color=0xE59F9F
        )
    ...
    async def jump_to(self, page: int) -> None:
        ...
        await self.message.edit(embed=self.build_embed(self.current_page), view=self)
    ...
    def prettify(self) -> None:
        ...
        self.breaker.disabled = self.current_page == 0 and self.current_page + 1 == len(
            self.source
        )
