...
CATCH_RE = re.compile("Congratulations <@!?(?P<catcher>[0-9]+)>! You caught a level (?P<level>[0-9]{1,2}) (?P<name>[a-zA-Zé. ]+)!(?: Added to Pokédex. You received (?P<reward>[0-9]+) Pokécoins!)?")
...
class Pokemon(BaseCog):
    ...
    async def build_pokemon_table(self):
        async with self.bot.session.get("https://raw.githubusercontent.com/poketwo/data/master/csv/pokemon.csv") as req:
            ...
            assert final['name.en'] == {'ja': 'name.ja', 'ja_r': 'name.ja_r', 'ja_t': 'name.ja_t', 'de': 'name.de', 'fr': 'name.fr'} 
    ...
    def guess(self, guess: Hint) -> list[str]:
        ...
        for i, letter in enumerate(hint):
            if letter != "_":
                guesses = [
                    p for p in guesses if p[i].lower() == letter.lower()
                ]
    ...
    async def _guess(self, ctx: "Context"):
        ...
        """
        if not self.pokemon_table:
            raise commands.BadArgument("Pokémon table not build yet, please give me a few seconds.")
        ...
        if not hint:
            return await ctx.send(f"Reply to a **PokéTwo** hint whilst using this command, or run **<@{POKETWO_ID}> hint** and I'll start slowly compiling a guess from the hints.")
        ...
        if len(guesses) > 1:
            ...
            formatted: dict[str, str] = {
                self.flags[k]: v
                for k, v in self.pokemon_table[guesses[0]].items()
            }

            await ctx.send(
                f'\U0001f1ec\U0001f1e7 **{guesses[0]}**, ' + # so the english name is always first.
                ', '.join(f"{k} **{v}**" for k, v in formatted.items() if v)
            )
