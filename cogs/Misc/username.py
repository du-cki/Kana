...
class Username(commands.Cog):
    ...
    async def add(self, ctx: KanaContext, username: str):
        ...
        if any(c in "!@#$%^&*()-+?=,<>/" for c in username):
            return await ctx.send("There are no special characters in usernames dummy") # returns when string has special characters

        _user = await self._checkUser(username)
        if _user is None:
            return await ctx.send("This username does not exist") # returns when the username does not exist
        
        if await self.db.find_one({"robloxUser": username}) is not None:
            return await ctx.send("Someone has already claimed this username") # returns when someone already has that username in the database
        ...
        if q is not None: # if author is already in the database, run this
            ...
            await self.db.update_one({"_id": ctx.author.id}, {"$push": {"robloxUser": _user}})
            await self.db.update_one({"_id": ctx.author.id}, {"$set": {"username": str(ctx.author)}}) # for my reference
    ...
    async def remove(self, ctx: KanaContext, username: str):
        ...
        await self.db.update_one({"_id": ctx.author.id}, {"$pull": {"robloxUser": username}})
